    ...
    <div class="blog-like-container">
        ...
        <div class="filter-container" v-show="!isHideSearch" :style="{height:`${filterContainerHeight}px`}">
            <el-form :inline="true">
                ...
                <el-input
                    clearable
                    style="width: 150px;"
                    v-model="searchParam.likePersonId"
                    placeholder="请输入点赞人"
                    @keyup.enter.native="handleFind"
                />
        ...
        <div class="table-container" :style="{height: `calc(100% - ${calcTableHeight}px)`}">
            ...
            >
                ...
                <!--
                     ...
                     -->
                <el-table-column label="文章id" min-width="100" align="center">
                    <template slot-scope="scope">
                        <span>{{ scope.row.blog_id }}</span>
                    </template>
                </el-table-column>

                <el-table-column label="点赞人" min-width="100" align="center">
                    <template slot-scope="scope">
                        <span>{{ scope.row.like_person_id }}</span>
                    </template>
                </el-table-column>
        ...
        <el-dialog :title="isUpdate ? '编辑':'增加'" :visible.sync="dialogFormVisible">
            <el-form :model="form">
                ...
                <el-row>
                    <el-col :span="12">
                        <el-form-item label="文章id" label-width="120px" prop="blogId">
                            <el-input v-model="form.blogId" auto-complete="off"></el-input>
                        </el-form-item>
                    </el-col>
                </el-row>
                ...
                <el-row>
                    <el-col :span="12">
                        <el-form-item label="点赞人" label-width="120px" prop="likePersonId">
                            <el-input v-model="form.likePersonId" auto-complete="off"></el-input>
                        </el-form-item>
                    </el-col>
                </el-row>
...
*/
import {blogLikeApi} from '/src/api/blogLike'
    ...
    */
    ...
    methods: {
        ...
        handleEdit(row) {
            ...
            */
            this.form = {
                uid: row.uid,
                blogId: row.blog_id,
                likePersonId: row.like_person_id,
        ...
        async deleteConfirmHandle() {
            let {data} = await blogLikeApi.deleteBlogLikeByUid(this.selectIds)
            if (data.code === 1) {
                this.$message({
                    message: '删除成功',
                    type: 'success',
        ...
        async submitForm() {
            ...
            if (!this.isUpdate) { //若果是新增
                data = await blogLikeApi.saveBlogLike(this.form)
            } else { // 如果是编辑
                data = await blogLikeApi.updateBlogLikeByUid(this.form)
            }
        ...
        async getList() {
            let {data} = await blogLikeApi.queryBlogLikePage(this.searchParam)
