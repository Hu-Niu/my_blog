<template>
    <div class="blog-container">
        ...
        <div class="feature-btns" :style="{height: `${featureBtnsHeight}px`}">
            <el-row :gutter="10">
                <el-col :span="1.5">
                    <el-button class="filter-item" type="primary" @click="showEditorHandle" icon="el-icon-edit"
                    >添加博客
                    </el-button>
                </el-col>
        ...
        <div class="table-container" :style="{height: `calc(100% - ${calcTableHeight}px)`}">
            ...
            >
                ...
                <el-table-column label="标题" min-width="160" align="center">
                    <template slot-scope="scope">
                        <span @click="onClick(scope.row)" style="cursor:pointer;">{{ scope.row.blog_title }}</span>
                    </template>
                </el-table-column>
                ...
                <el-table-column label="是否原创" min-width="100" align="center">
                    <template slot-scope="scope">
                        <el-tag v-if="scope.row.is_original==1" type="success">原创</el-tag>
                ...
                <el-table-column label="是否私密" min-width="100" align="center">
                    <template slot-scope="scope">
                        <el-tag v-if="scope.row.is_private==1" type="success">私密</el-tag>
                ...
                <el-table-column label="推荐等级" min-width="100" align="center" prop="level">
                    <template slot-scope="scope">
                        <el-tag v-if="scope.row.recommend_level==1" type="success">一级推荐</el-tag>
                        <el-tag v-if="scope.row.recommend_level==2" type="info">二级推荐</el-tag>
                        <el-tag v-if="scope.row.recommend_level==3" type="info">三级推荐</el-tag>
                        <el-tag v-if="scope.row.recommend_level==4" type="info">四级推荐</el-tag>
                        <el-tag v-if="scope.row.recommend_level==-1" type="info">不推荐</el-tag>
                    </template>
                </el-table-column>

                <el-table-column label="点击数" min-width="90" align="center" prop="clickCount">
                    <template slot-scope="scope">
                        <span>{{ scope.row.clicks }}</span>
                    </template>
                </el-table-column>

                <el-table-column label="开启评论" min-width="100" align="center" prop="openComment">
                    <template slot-scope="scope">
                        <el-tag v-if="scope.row.is_open_comment==1" type="success">开启</el-tag>
                        <el-tag v-if="scope.row.is_open_comment==2" type="info">关闭</el-tag>
                    </template>
                </el-table-column>

                <el-table-column label="发布状态" min-width="100" align="center">
                    <template slot-scope="scope">
                        <el-tag v-if="scope.row.blog_status==1" type="success">发布</el-tag>
...
export default {
    ...
    methods: {
        ...
        beforeUpload(file) {
            ...
            this.fileExtraData.blogTitle = `${file.name} ${dayjs().format('YYYY-MM-DD HH:mm:ss:SSS')}`
        },
        fileSuccess(res) {
            // 每次先清空上一次的timer
        ...
        async saveBlogHandle(flag) {
            ...
            let params = {
                ...
                originAddress: +this.form.isOriginal === 1 ? '' : this.form.originAddress, //为原创时，来源地址置为空
            }
            ...
            if (this.isContentChange) {
                ...
                this.$confirm('博客尚未保存，是否保存为草稿？', '提示', {
                    ...
                    .then(async () => { // 保存为草稿
                        ...
                        this.form.blogContent = findData.content
                        this.form.uid = findData.blogUid
                        this.form.blogTitle = findData.title.trim() === "" ? dayjs().format('YYYY-MM-DD HH:mm:ss') : findData.title.trim()
