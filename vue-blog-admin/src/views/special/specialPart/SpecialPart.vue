<template>
    <!--
      ...
      替换方式：模块名变为连字符类型
        blogSort -> blog-sort
    -->
    <div class="special-part-container">
        <!--  筛选框  -->
        <div class="filter-container" v-show="!isHideSearch" :style="{height:`${filterContainerHeight}px`}">
            <el-form :inline="true">
                ...
                <el-input
                    clearable
                    style="width: 150px;"
                    v-model="searchParam.partTitle"
                    placeholder="请输入当前部分的大标题"
                    @keyup.enter.native="handleFind"
                />
                ...
                <el-input
                    clearable
                    style="width: 150px;"
                    v-model="searchParam.partSummary"
                    placeholder="请输入当前部分的概述"
                    @keyup.enter.native="handleFind"
                />

                <el-select
                    v-model="searchParam.specialId"
                    clearable
                    placeholder="请选择属于哪个专题"
                    style="width: 150px;"
                >
                    <el-option
        ...
        <div class="table-container" :style="{height: `calc(100% - ${calcTableHeight}px)`}">
            ...
            >
                ...
                <el-table-column label="属于哪个专题" min-width="100" align="center">
                    <template slot-scope="scope">
                ...
                <!--
                     ...
                     -->
                <el-table-column label="第几部分" min-width="100" align="center">
                    <template slot-scope="scope">
                        <span>{{ scope.row.part_name }}</span>
                    </template>
                </el-table-column>

                <el-table-column label="当前部分的大标题" min-width="100" align="center">
                    <template slot-scope="scope">
                        <span>{{ scope.row.part_title }}</span>
                    </template>
                </el-table-column>

                <el-table-column label="当前部分的概述" min-width="100" align="center">
                    <template slot-scope="scope">
                        <span>{{ scope.row.part_summary }}</span>
                    </template>
                </el-table-column>
        ...
        <el-dialog :title="isUpdate ? '编辑':'增加'" :visible.sync="dialogFormVisible">
            <el-form :model="form">
                ...
                <el-row>
                    <el-col :span="12">
                        <el-form-item label="第几部分" label-width="120px" prop="partName">
                            <el-input v-model="form.partName" auto-complete="off"></el-input>
                        </el-form-item>
                    </el-col>
                </el-row>
                ...
                <el-row>
                    <el-col :span="12">
                        <el-form-item label="当前部分的大标题" label-width="120px" prop="partTitle">
                            <el-input v-model="form.partTitle" auto-complete="off"></el-input>
                        </el-form-item>
                    </el-col>
                </el-row>
                ...
                <el-row>
                    <el-col :span="12">
                        <el-form-item label="当前部分的概述" label-width="120px" prop="partSummary">
                            <el-input v-model="form.partSummary" auto-complete="off"></el-input>
                        </el-form-item>
                    </el-col>
                </el-row>
                ...
                <el-row>
                    <el-col :span="12">
                        <el-form-item label="属于哪个专题" label-width="120px" prop="specialId">
                            <el-select
...
*/
import {specialPartApi} from '/src/api/specialPart'
    ...
    */
    ...
    methods: {
        ...
        async getSpecialList() {
            let {data} = await specialApi.querySpecialAll()
            console.log(data, 'getSpecialList')
        ...
        async deleteConfirmHandle() {
            let {data} = await specialPartApi.deleteSpecialPartByUid(this.selectIds)
            if (data.code === 1) {
                this.$message({
                    message: '删除成功',
                    type: 'success',
        ...
        async submitForm() {
            ...
            if (!this.isUpdate) { //若果是新增
                data = await specialPartApi.saveSpecialPart(this.form)
            } else { // 如果是编辑
                data = await specialPartApi.updateSpecialPartByUid(this.form)
            }
        ...
        async getList() {
            let {data} = await specialPartApi.querySpecialPartPage(this.searchParam)
