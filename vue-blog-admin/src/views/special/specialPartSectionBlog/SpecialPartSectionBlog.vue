    ...
    <div class="special-part-section-blog-container">
                ...
                <el-form :inline="true">
                    ...
                    >
                        <el-option
                            ...
                            :key="item.uid"
                            :label="item.special_name+'/'+item.part_name+'/'+item.section_title"
                            :value="item.uid"
                        ></el-option>
                    </el-select>
                ...
                >
                    ...
                    <el-table-column label="属于哪个章节" min-width="100" align="center">
                        <template slot-scope="scope">
                            <span>{{
                                ...
                                }}/{{ scope.row.part_name }}/{{ scope.row.section_title }}</span>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column label="操作" fixed="right" min-width="150">
                        <template slot-scope="scope">
                            ...
                            <el-button @click="handleSingleDelete(scope.row,scope.$index+1)" type="danger" size="small">
                                删除
                            </el-button>
                        </template>
                    </el-table-column>
        ...
        <el-dialog :title="isUpdate ? '编辑':'增加'" :visible.sync="dialogFormVisible">
            <el-form :model="form">
                ...
                <el-row>
                    <el-col :span="12">
                        <el-form-item label="属于哪个章节" label-width="120px" prop="partSectionId">
                            <el-select
    ...
    */
    ...
    methods: {
        ...
        async deleteConfirmHandle() {
            let {data} = await specialPartSectionBlogApi.deleteSpecialPartSectionBlogByUid(this.selectIds)
            if (data.code === 1) {
                this.$message({
                    message: '删除成功',
                    type: 'success',
        ...
        async submitForm() {
            ...
            if (!this.isUpdate) { //若果是新增
                data = await specialPartSectionBlogApi.saveSpecialPartSectionBlog(this.form)
            } else { // 如果是编辑
                data = await specialPartSectionBlogApi.updateSpecialPartSectionBlogByUid(this.form)
            }
        ...
        async getList() {
            let {data} = await specialPartSectionBlogApi.querySpecialPartSectionBlogPage(this.searchParam)
