<template>
    <!--
      ...
      替换方式：模块名变为连字符类型
        blogSort -> blog-sort
    -->
    <div class="special-part-section-container">
        <!--  筛选框  -->
        <div class="filter-container" v-show="!isHideSearch" :style="{height:`${filterContainerHeight}px`}">
            <el-form :inline="true">
                ...
                <el-input
                    clearable
                    style="width: 150px;"
                    v-model="searchParam.sectionTitle"
                    placeholder="请输入当前章节标题"
                    @keyup.enter.native="handleFind"
                />

                <el-select
                    v-model="searchParam.specialPartId"
                    clearable
                    placeholder="请选择属于哪个专题部分"
                    style="width: 150px;"
                >
                    <el-option
                        v-for="item in specialPartList"
                        :key="item.uid"
                        :label="item.special_name+'/'+item.part_name"
                        :value="item.uid"
                    ></el-option>
                </el-select>
        ...
        <div class="table-container" :style="{height: `calc(100% - ${calcTableHeight}px)`}">
            ...
            >
                ...
                <el-table-column label="属于哪个专题部分" min-width="100" align="center">
                    <template slot-scope="scope">
                        <span>{{ scope.row.special_name + '/' + scope.row.part_name }}</span>
                    </template>
                </el-table-column>
                ...
                <!--
                     ...
                     -->
                <el-table-column label="当前章节标题" min-width="100" align="center">
                    <template slot-scope="scope">
                        <span>{{ scope.row.section_title }}</span>
                    </template>
                </el-table-column>
        ...
        <el-dialog :title="isUpdate ? '编辑':'增加'" :visible.sync="dialogFormVisible">
            <el-form :model="form">
                ...
                <el-row>
                    <el-col :span="12">
                        <el-form-item label="当前章节标题" label-width="120px" prop="sectionTitle">
                            <el-input v-model="form.sectionTitle" auto-complete="off"></el-input>
                        </el-form-item>
                    </el-col>
                </el-row>
                ...
                <el-row>
                    <el-col :span="12">
                        <el-form-item label="属于哪个专题部分" label-width="120px" prop="specialPartId">
                            <el-select
                                v-model="form.specialPartId"
                                clearable
                                placeholder="请选择属于哪个专题部分"
                                style="width:100%"
                            >
...
export default {
    ...
    */
    ...
    methods: {
        ...
        async getSpecialPartList() {
            let {data} = await specialPartApi.querySpecialPartAll()
            this.specialPartList = data.data
        },
        ...
        handleEdit(row) {
            ...
            */
            this.form = {
                ...
                specialPartId: row.special_part_id,
        ...
        async deleteConfirmHandle() {
            let {data} = await specialPartSectionApi.deleteSpecialPartSectionByUid(this.selectIds)
            if (data.code === 1) {
                this.$message({
                    message: '删除成功',
                    type: 'success',
        ...
        async submitForm() {
            ...
            if (!this.isUpdate) { //若果是新增
                data = await specialPartSectionApi.saveSpecialPartSection(this.form)
            } else { // 如果是编辑
                data = await specialPartSectionApi.updateSpecialPartSectionByUid(this.form)
            }
        ...
        async getList() {
            let {data} = await specialPartSectionApi.querySpecialPartSectionPage(this.searchParam)
